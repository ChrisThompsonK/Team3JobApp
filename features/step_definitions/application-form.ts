...
Given('I am logged in as a test user', async function () {
  ...
  const isLoggedIn = await world.page.locator('[data-testid="user-menu"]').isVisible().catch(() => false);
...
Given('I navigate to the job application page for job {string}', async function (jobId: string) {
  if (!world.page) throw new Error('Page not initialized');
...
When('I view the application form', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
When('I view the job application page', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
When('I attempt to submit the form without filling any fields', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
When('I enter an invalid email format {string}', async function (email: string) {
  if (!world.page) throw new Error('Page not initialized');
...
When('I fill the application form with valid data:', async function (dataTable: DataTable) {
  if (!world.page) throw new Error('Page not initialized');
...
When('I test phone codes for different countries:', async function (dataTable: DataTable) {
  if (!world.page) throw new Error('Page not initialized');
  ...
  for (const testCase of testCases) {
    ...
    await world.page.fill('input#email', `test-${world.timestamp}-${testCase.countryCode.replace('+', '')}-@example.com`, { timeout: 10000 });
...
When('I fill the form with valid data including CV upload', async function () {
  if (!world.page) throw new Error('Page not initialized');
  ...
  await world.page.selectOption('select#yearsOfExperience', testData.yearsOfExperience, { timeout: 10000 });
  await world.page.fill('textarea#coverLetter', testData.coverLetter, { timeout: 10000 });
  await world.page.check('input[name="acceptTerms"]', { timeout: 10000 });
});

When('I upload a PDF file for the CV', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  await world.page.setInputFiles('input#cv', {
...
When('I fill all required fields with valid data', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
When('I fill the form with data:', async function (dataTable: DataTable) {
  if (!world.page) throw new Error('Page not initialized');
...
When('I attempt to submit without all required fields', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  await world.page.click('button#submitBtn', { timeout: 10000 });
});

When('I fill the form with an invalid LinkedIn URL {string}', async function (url: string) {
  if (!world.page) throw new Error('Page not initialized');
...
When('I fill the form with special characters in the cover letter:', async function (dataTable: DataTable) {
  if (!world.page) throw new Error('Page not initialized');
...
When('I navigate directly to the apply page', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  const response = await world.page.goto(`${world.baseURL}/jobs/${world.testJobId}/apply`, { waitUntil: 'networkidle' });
});

When('I submit a valid application to a job', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
When('the application is successfully submitted', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  await world.page.waitForURL(`/jobs/${world.testJobId}/details`, { timeout: 5000 });
});

When('I navigate back to apply for the same job again', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  await world.page.goto(`${world.baseURL}/jobs/${world.testJobId}/apply`);
});

When('I submit another application with the same details', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the form should display all required fields:', async function (dataTable: DataTable) {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the job role card should display relevant information', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the card text should be visible', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the form should still be visible', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('browser validation should prevent submission', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the form should be successfully submitted', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('I should be redirected to the job details page', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  await expect(world.page).toHaveURL(new RegExp(`/jobs/${world.testJobId}/details`));
});

Then('each country code should be properly selected in the form', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  // Already verified in the When step
});

Then('phone numbers should be accepted for each country', async function () {
  if (!world.page) throw new Error('Page not initialized');
  
  // Already verified in the When step
});

Then('the file should be successfully attached to the form', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the file input should have a value', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the form data should be preserved in the form fields', async function () {
  if (!world.page) throw new Error('Page not initialized');
  ...
  expect(await world.page.inputValue('textarea#coverLetter')).toContain('This is my cover letter text');
  expect(await world.page.inputValue('textarea#additionalComments')).toContain('Additional information here');
});

Then('browser validation should detect the invalid URL', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the special characters should be properly preserved', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the form should accept the special character content', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('an error message should indicate that I\'ve already applied to this job', async function () {
  if (!world.page) throw new Error('Page not initialized');
...
Then('the duplicate application should be prevented', async function () {
  if (!world.page) throw new Error('Page not initialized');
