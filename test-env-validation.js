...
async function testMissingEnvVar(varName) {
  return new Promise((resolve) => {
    ...
    requiredAuthVars.forEach(v => delete env[v]);
    ...
    const timeout = setTimeout(() => {
      child.kill();
      console.log(`  ❌ FAIL: App did not exit (should have thrown error for missing ${varName})\n`);
      resolve(false);
    }, 3000);
    ...
    child.on('exit', (code) => {
      ...
      const hasError = combinedOutput.includes('FATAL') || 
                       combinedOutput.includes(`${varName}`) ||
                       code !== 0;
...
async function testAllVarsPresent() {
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      const combinedOutput = output + errorOutput;
      const started = combinedOutput.includes('Server') || 
