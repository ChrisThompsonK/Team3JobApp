...
When('I fill in the registration form with:', async (dataTable: DataTable) => {
  if (!loginWorld.page) throw new Error('Page not initialized');
...
When('I click the logout button', async () => {
  ...
  const logoutButton = loginWorld.page
    .locator('button:has-text("Logout"), a:has-text("Logout"), [data-testid="logout-btn"]')
    .first();
...
Then('I should be logged in', async () => {
  ...
  const userMenu = loginWorld.page
    .locator('[data-testid="user-menu"], .user-profile, .user-menu')
    .first();
