...
test.describe('Job Roles', () => {
  test('should display job roles list', async ({ page }) => {
    ...
    const hasJobList = await page.locator('table, .job-list, .job-card, [data-testid*="job"]').count() > 0;
  ...
  test('should view job role details', async ({ page }) => {
    ...
    const jobLink = page.locator('a[href*="job-role"], button:has-text("View"), button:has-text("Details")').first();
