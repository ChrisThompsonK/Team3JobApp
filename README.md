...
This project uses multiple testing frameworks for comprehensive test coverage:

### **Test Types**
- **Unit Tests**: Individual functions and components using Vitest
- **Integration Tests**: Service layer interactions using Vitest
- **E2E Tests**: End-to-end user workflows using Cucumber and Playwright
- **POM Tests**: Page Object Model tests using Playwright
...
```bash
# Interactive test runner (watches for changes, re-runs on save)
npm run test

# Run all tests once (CI/CD mode, no watch)
npm run test:run
...
# Run only integration tests once
npm run test:integration

# Visual UI for integration tests only
...
#### **POM Tests (Playwright)**
```bash
# Run Playwright Page Object Model tests (headless)
npm run test:pom:run

# Run Playwright tests in headed mode (see browser)
npm run test:pom:headed

# Visual UI for Playwright tests
npm run test:pom:ui
```

#### **Complete Test Suites**
```bash
# Run all tests in sequence (unit, E2E, POM)
npm run test:all

# Run all tests and generate comprehensive reports
npm run test:all:with-report

# Generate test report (aggregates results from all test types)
npm run test:report
```
...
- **Formats**: Text (terminal), JSON, HTML
- **View Coverage**: Open `/coverage/index.html` in browser
- **Command**: `npm run test:coverage`

#### **Vitest Reports**
- **JSON Report**: `vitest-report.json`
- **View in Terminal**: Run `npm run test:run` to see results
- **HTML UI**: Run `npm run test:ui` for interactive browser view

#### **E2E Reports**
- **Cucumber HTML**: `cucumber-report.html` (generated by `npm run test:e2e:report`)
- **View**: Open `cucumber-report.html` in browser

#### **Playwright Reports**
- **HTML Report**: `/playwright-report/index.html`
- **Results JSON**: `/playwright-report/results.json`
- **View**: Run `npm run test:pom:run`, then check the generated report

#### **Aggregated Test Reports**
- **Location**: `/test-reports` directory
- **Generated by**: `npm run test:report`
- **Contains**: Summary of all test runs with timestamps

### **Test Structure**
- **Unit Tests**: `src/**/*.{test,spec}.ts`
- **Integration Tests**: `tests/integration/**/*.{test,spec}.ts`
- **E2E Cucumber Tests**: `tests/e2e/features/` (feature files) and `tests/e2e/step_definitions/`
- **POM Tests**: `tests/e2e/**/*-pom.spec.ts` (Page Object Model)
- **Support Files**: `tests/e2e/support/` (hooks, world configuration)
...
#### **E2E Tests (Cucumber)**
```gherkin
# tests/e2e/features/example.feature
...
```bash
# Commands run in CI/CD (GitHub Actions, etc.)
npm run type-check      # Type check
npm run ci              # Biome CI check
